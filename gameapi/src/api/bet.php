<?php
include "./conn.php";
$connect = new PDO("mysql:host=localhost;dbname=smarthubb_db", "smarthubb_db", "smarthubb_db");
...
array_push($data, ['status' => $status],['amount' => $amount],['ans' => $ans],['user' => $user]);
	...
	$sum2 = mysqli_fetch_assoc($optres222);
	$total2 = $sum2['total22'];
	$opt="SELECT SUM(amount) as total FROM `abbetting` WHERE username='$user' AND res='success'";
$optres=$conn->query($opt);
$sum= mysqli_fetch_assoc($optres);
if($sum['total']==""){
    $bonus=0;
	...
	$opt1="SELECT SUM(amount) as total1 FROM `betting` WHERE username='$user' AND res='success' ";
	...
	$opt2="SELECT SUM(amount) as total2 FROM `saprebetting` WHERE username='$user' AND res='success'";
	...
	$opt3="SELECT SUM(amount) as total3 FROM `emredbetting` WHERE username='$user' AND res='success'";
	...
	$opt4="SELECT SUM(amount) as total4 FROM `beconebetting` WHERE username='$user' AND res='success'";
	...
	if($total2>0){
	if(($bonus1+$bonus2+$bonus3+$bonus4)*3.5>$_GET['amount']){
	if($wcount<=4){
	...
	if($result===true && $res1===true){
	    ...
	    $a=($bonus1+$bonus2+$bonus3+$bonus4)*3.5;
        $status="Only Winnings are withdrawable  ";
	}
	}else{
